'''
    www.aifes.ai
    Copyright (C) 2020-2024 Fraunhofer Institute for Microelectronic Circuits and Systems. All rights reserved.
...
class Layer(Enum):
    DENSE = 1
    DENSE_WT = 2 # Dense layer with transposed weights matrix
    ELU = 3
    LEAKY_RELU = 4
    RELU = 5
    SIGMOID = 6
...
def print_flatbuffer_c_style(flatbuffer, target_alignment=4, mutable=True, elements_per_line=-1, byteorder='little'):
    ...
    print("const uint32_t parameter_memory_size = {};".format(len(flatbuffer_turned)))
    ...
    print(" model_parameters[", int(len(flatbuffer_turned) / target_alignment) , "] = {\n    ", end='')
    print("0x", end='')
